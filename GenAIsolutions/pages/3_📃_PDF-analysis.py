...
if (
    ...
    def download_pdf(file_name):
        ...
        # Convert the PDF data to base64 encoding
        base64_pdf = base64.b64encode(pdf_data).decode('UTF-8')
    
        # Create a download link for the PDF file
        href = f'<a href="data:application/pdf;base64,{base64_pdf}" download="{file_name}">Download {file_name}</a>'
    
        # Display the download link in Streamlit
        st.markdown(href, unsafe_allow_html=True)
    
    st.write(f'**{"Download a sample file to start"}**')
    # Create a list of available PDF files
    pdf_files = ["IntroductiontoSoftwareDevelopment.pdf", "bank-statement.pdf", "payslip.pdf"]
    
    # Display the available PDF files as a selectbox
    selected_file = st.selectbox("Select a PDF file to download", pdf_files)
    
    # Call the download_pdf function with the selected file
    download_pdf(selected_file)
